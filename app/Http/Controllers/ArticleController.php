...
$query = Article::where('designation', 'like', '%' . $filtre . '%') ->with('scategories') ;// une relation définie avec scategories ->orderBy('id', 'desc'); // Tri descendant par ID 
// Pagination 
$totalArticles = $query->count(); 
$articles = $query->skip(($page - 1) * $pageSize) ->take($pageSize) ->get(); 
